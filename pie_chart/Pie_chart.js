  ...
  main.variable(observer("chart")).define("chart", ["partition","data","d3","width","height","color","arc","format","radius"], function(partition,data,d3,width,height,color,arc,format,radius)
{
  ...
  const svg = d3.create("svg")
      .attr("viewBox", [0, 0, width, height]) // Utilisez height au lieu de width pour la hauteur
      .style("font", "10px sans-serif");

  const g = svg.append("g")
      .attr("transform", `translate(${width / 2},${height / 2})`); // Utilisez height au lieu de width pour la hauteur
...
750  // Réglez la hauteur de la fenêtre ici
)});
  main.variable(observer("radius")).define("radius", ["width"], function(width){return(
width / 9  // Vous pouvez ajuster le dénominateur pour changer la taille du cercle
)});
